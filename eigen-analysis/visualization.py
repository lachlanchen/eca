...
def visualize_mnist_eigenfeatures(model, output_dir='mnist_visualizations'):
    """
    ...
    # Save the L matrix as a heatmap (transposed and wider) with light gray background
    ...
    plt.ylabel('Classes (Digits 0-9)', color='black', fontsize=16)
    ...
    # Generate a visualization of eigenfeatures per class (stacked bar chart)
    ...
    # Create the stacked bars (shared on bottom, pure on top)
    ...
    # Add labels and styling with dark text for better readability
    plt.xlabel('Digit Class', fontsize=16, color='black')
...
def visualize_clustering_results(X, y_true, remapped_predictions, loss_history, psi_final, num_epochs, 
    ...
    # Calculate ARI score using sklearn
