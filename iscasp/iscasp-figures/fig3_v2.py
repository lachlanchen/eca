import numpy as np
import matplotlib.pyplot as plt
from sklearn.datasets import load_iris

# Load the Iris dataset
iris = load_iris()
X = iris.data  # Original data
y = iris.target  # Class labels

# Define the transformation matrix P (from your ECA model)
P = np.array([[-0.4772,  0.4370,  0.4973, -0.1016],
              [-0.7707,  0.4478, -0.4387, -0.0193],
              [-0.3080, -0.5219,  0.7055,  0.2904],
              [ 0.2888,  0.5798, -0.2498,  0.9513]])

# Define the mapping matrix L (from your ECA model)
L = np.array([[1., 1., 0.],
              [1., 0., 0.],
              [0., 1., 1.],
              [0., 0., 1.]])

# Transform the data using P
Psi = X @ P

# Prepare the figure with three subplots in a row
fig = plt.figure(figsize=(18, 5))

# Set the font sizes
title_fontsize = 18  # Slightly downscaled title font size
axis_label_fontsize = 16  # Adjusted font size for axis labels

# Class 1 vs Class 2
ax1 = fig.add_subplot(131, projection='3d')
indices = np.where((y == 0) | (y == 1))[0]
X_plot = Psi[indices][:, [0, 1, 2]]  # Use dimensions associated with classes 1 and 2
y_plot = y[indices]
scatter = ax1.scatter(X_plot[:, 0], X_plot[:, 1], X_plot[:, 2], c=y_plot, cmap='cividis', s=50)
ax1.set_title('Class 1 vs Class 2', fontsize=title_fontsize)
ax1.set_xlabel('Eigenfeature 1', fontsize=axis_label_fontsize)
ax1.set_ylabel('Eigenfeature 2', fontsize=axis_label_fontsize)
ax1.set_zlabel('Eigenfeature 3', fontsize=axis_label_fontsize)
ax1.tick_params(axis='both', which='both', labelsize=0, length=0)  # Remove tick labels and ticks
ax1.grid(False)

# Class 2 vs Class 3
ax2 = fig.add_subplot(132, projection='3d')
indices = np.where((y == 1) | (y == 2))[0]
X_plot = Psi[indices][:, [0, 2, 3]]  # Excluding the 2nd dimension (1-indexed)
y_plot = y[indices]
scatter = ax2.scatter(X_plot[:, 0], X_plot[:, 1], X_plot[:, 2], c=y_plot, cmap='cividis', s=50)
ax2.set_title('Class 2 vs Class 3', fontsize=title_fontsize)
ax2.set_xlabel('Eigenfeature 1', fontsize=axis_label_fontsize)
ax2.set_ylabel('Eigenfeature 3', fontsize=axis_label_fontsize)
ax2.set_zlabel('Eigenfeature 4', fontsize=axis_label_fontsize)
ax2.tick_params(axis='both', which='both', labelsize=0, length=0)
ax2.grid(False)

# Class 1 vs Class 3
ax3 = fig.add_subplot(133, projection='3d')
indices = np.where((y == 0) | (y == 2))[0]
X_plot = Psi[indices][:, [0, 1, 3]]  # Use relevant dimensions
y_plot = y[indices]
scatter = ax3.scatter(X_plot[:, 0], X_plot[:, 1], X_plot[:, 2], c=y_plot, cmap='cividis', s=50)
ax3.set_title('Class 1 vs Class 3', fontsize=title_fontsize)
ax3.set_xlabel('Eigenfeature 1', fontsize=axis_label_fontsize)
ax3.set_ylabel('Eigenfeature 2', fontsize=axis_label_fontsize)
ax3.set_zlabel('Eigenfeature 4', fontsize=axis_label_fontsize)
ax3.tick_params(axis='both', which='both', labelsize=0, length=0)
ax3.grid(False)

plt.tight_layout()
plt.show()

