...
scatter = ax0.scatter(pca_transformed[:, 0], pca_transformed[:, 1], pca_transformed[:, 2], c=y, 
                      cmap='cividis', s=50, edgecolor='k', linewidth=0.5)
...
y_plot = y[indices]
scatter = ax1.scatter(X_plot[:, 0], X_plot[:, 1], X_plot[:, 2], c=y_plot, 
                      cmap='cividis', s=50, edgecolor='k', linewidth=0.5)
...
y_plot = y[indices]
scatter = ax2.scatter(X_plot[:, 0], X_plot[:, 1], X_plot[:, 2], c=y_plot, 
                      cmap='cividis', s=50, edgecolor='k', linewidth=0.5)
...
y_plot = y[indices]
scatter = ax3.scatter(X_plot[:, 0], X_plot[:, 1], X_plot[:, 2], c=y_plot, 
                      cmap='cividis', s=50, edgecolor='k', linewidth=0.5)
...
# L_matrix heatmap with formatting to hide zero entries and add white lines for spacing
ax4 = fig.add_subplot(155)
sns.heatmap(L_matrix, annot=True, mask=mask, cmap='Blues', fmt=".0f", cbar=False,
            ax=ax4, annot_kws={"size": 14}, linewidths=2, linecolor='white', square=True)
ax4.set_xticks([])
ax4.set_yticks([])
